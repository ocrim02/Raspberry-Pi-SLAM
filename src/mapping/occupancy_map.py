...
class OccupancyGrid:
    ...
    def updateOccupancyGrid(self, reading, dTheta = 0, update=True, replace=False):
        ...
        wall_half_thickness = self.wallThickness / 2
        ...
        for i in range(self.numSamplesPerRev):
            ...
            if rMeasure[i] < self.lidarMaxRange:
                emptyIdx = np.argwhere(rAtSpokeDir < rMeasure[i] - wall_half_thickness)
            else:
                emptyIdx = []
            occupiedIdx = np.argwhere((rAtSpokeDir > rMeasure[i] - wall_half_thickness) & (rAtSpokeDir < rMeasure[i] + wall_half_thickness))
            ...
            if update:
                ...
                if len(emptyIdx) != 0:
                    self.occupancyGridTotal[yEmptyIdx, xEmptyIdx] += factor1
                if len(occupiedIdx) != 0:
                    self.occupancyGridVisited[yOccupiedIdx, xOccupiedIdx] += factor2
                    self.occupancyGridTotal[yOccupiedIdx, xOccupiedIdx] += factor2
            else:
    ...
    def addOccupancyGridToPlot(self, ocgMap, xRange = None, yRange= None):
        if xRange is None or xRange[0] < self.mapXLim[0] or xRange[1] > self.mapXLim[1]:
            xRange = self.mapXLim
        if yRange is None or yRange[0] < self.mapYLim[0] or yRange[1] > self.mapYLim[1]:
            yRange = self.mapYLim
        ...
        ogMap = ogMap[yIdx[0]: yIdx[1], xIdx[0]: xIdx[1]]
        ogMap = np.flipud(1 - ogMap)
        ocgMap.set_data(ogMap)


def updateTrajectoryPlot(matchedReading, xTrajectory, yTrajectory):
...
def replaceElementInTrajectoryPlot(matchedReading, id, xTrajectory, yTrajectory):
    x, y, theta, range = matchedReading['x'], matchedReading['y'], matchedReading['theta'], matchedReading['range']
    xTrajectory[id] = x
    yTrajectory[id] = y
